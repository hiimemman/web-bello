    ...
    try{
        ...
        if (!empty($Password)) {
            ...
            if ($Password == $Password1) {
                ...
                $sql = "UPDATE `tbl_hoa` SET `image_url` = '$ImageURL', `firstname` = '$Firstname', `lastname` = '$Lastname', `password` = '$Password', `email` = '$Email', `position` = '$Position', `contact` = '$Contact', `email` = '$Email', `password` = '$Password' WHERE `tbl_hoa`.`id`= '$ID';"; 
...
if (isset($_FILES['image_url']) && $_FILES['image_url']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($file['tmp_name'], $uploadDir . $filename)) {
        ...
        $sql = "UPDATE tbl_hoa SET image_url = '$uploadDir$filename' WHERE id = '$ID'";
        $result = mysqli_query($con, $sql);
